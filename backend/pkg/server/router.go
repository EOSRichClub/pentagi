...
func NewRouter(
	...
	flowService := services.NewFlowService(orm, providers, controller, subscriptions, cfg.DataDir)
...
func setFlowFilesGroup(parent *gin.RouterGroup, svc *services.FlowFileService) {
	flowFilesGroup := parent.Group("/flows/:flowID/files")
	{
		...
		flowFilesGroup.POST("/mkdir", svc.MkdirFlowFiles)
		...
		flowFilesGroup.POST("/sync", svc.SyncFlowFiles)
